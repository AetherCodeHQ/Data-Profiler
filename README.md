...
> Data-Profiler - Open source tool by AetherCodeHQ

`analysis` `cli` `data` `golang` `profiling`

## What is it?

**Data Profiler** is a data tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Data-Profiler.git
cd Data-Profiler
go build -o Data-Profiler .
```
...
```bash
# Basic usage
./Data-Profiler --path ./target

# With options
./Data-Profiler --path ./target --format json --output report.json

# Verbose mode
./Data-Profiler --path ./target --verbose
```
...
### Basic scan
```bash
./Data-Profiler --path ./my-project
```

### JSON report
```bash
./Data-Profiler --path ./my-project --format json --output report.json
```
...
- name: Run Data Profiler
  run: ./Data-Profiler --path . --format json --output report.json
```
